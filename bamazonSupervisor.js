...
function createDepartment() {
  inquirer
    ...
    .then(answers => {
      var queryAdd =
        "INSERT INTO departments (dept_name, over_head_costs) VALUES (?, ?)";
      connection.query(queryAdd, [answers.deptName, answers.overhead], function(
        error
      ) {
        if (error) {
          console.log(
            chalk.red(
              "There was an issue adding the department. Please try again."
            )
          );
        } else {
