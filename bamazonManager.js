    ...
    connection.query(queryProducts, function(error, results) {
        if (error) {
            ...
            inquirer.prompt([
                {
                    name: 'item_id',
                    message: chalk.cyan('Enter the "item_id" of the product that needs inventory added.'),
                    validate: function(value) {
                        if (/[0-9]/g.test(value) && value > 0 && value <= results.length) {
                            return true;
                        }
                ...
                {
                    name: 'count',
                    message: chalk.cyan('Enter the quantity of how much to increase the inventory of the product.'),
                    validate: function(value) {
                ...
                stockCount = parseInt(answers.count) + parseInt(results[answers.item_id - 1].stock_quantity);
                connection.query(queryUpdate, [stockCount, answers.item_id], function(error) {
                    if (error) {
                        console.log(chalk.red('There was an issue updating the quantity. Please try again.'));
                    } else {
    ...
    connection.query(deptQuery, function(error, results) {
        if (error) {
            ...
            inquirer.prompt([
                ...
                var queryAdd = 'INSERT INTO products (product_name, department_name, price, stock_quantity) VALUES (?, ?, ?, ?)';
                connection.query(queryAdd, [answers.productName, answers.deptName, answers.price, answers.count], function(error) {
                    if (error) {
                        console.log(chalk.red('There was an issue adding the product. Please try again.'));
                    } else {
